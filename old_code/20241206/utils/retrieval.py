...
class VectorStore:
    def __init__(self, vector_store_name='paper_vector_store', embed_model='BAAI/bge-small-en-v1.5') -> None:
        ...
        self.vector_store_path = os.path.join('../store', vector_store_name)
        ...
        if os.path.exists(self.vector_store_path):
            ...
            # 如果存储文件不存在，初始化为空
            print(f"在 {self.vector_store_path} 处未发现向量数据库，将初始化为空")
    ...
    def _load(self):
        ...
        加载已有的向量存储。
        """
        print(f"从 {self.vector_store_path} 加载向量数据库")
    ...
    def save(self):
        ...
        print(f"向量数据库保存至 {self.vector_store_path}")
    ...
    def add_documents(self, documents):
        ...
        # 如果有新文档，更新向量存储
        if unique_documents:
            print(f"向向量数据库中添加 {len(unique_documents)} 条新信息")
