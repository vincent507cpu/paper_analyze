...
def main_loop():
    with gr.Blocks() as demo:
        gr.Markdown("<h1 style='text-align: center;'>arXiv 知识问答系统</h1>")
        start_prompt = gr.Markdown("**请发起新的查询**", visible=True)

        query_input = gr.Textbox(label="输入查询")
        submit_query_button = gr.Button("开始查询")
        academic_response = gr.Textbox(label="学术查询结果", interactive=False)
        logs = gr.Textbox(label="实时日志", interactive=False, lines=15)
        confirm_button = gr.Button("对结果满意，继续", visible=False)
        deny_button = gr.Button("搜索互联网寻找信息", visible=False)
        ...
        def preprocess_and_query(query, current_logs, dialog_history):
            ...
            if rewritten_query:
                ...
                return "", updated_logs, gr.update(visible=False), gr.update(visible=False), updated_logs, gr.update(visible=False), dialog_history
        ...
        submit_query_button.click(
            preprocess_and_query,
            inputs=[query_input, log_state, dialog_state],
            outputs=[academic_response, logs, confirm_button, deny_button, log_state, start_prompt, dialog_state],
        )

        confirm_button.click(
            reset_to_new_query,
            inputs=[log_state, dialog_state],
            outputs=[query_input, log_state, start_prompt, logs, dialog_state],
        )
